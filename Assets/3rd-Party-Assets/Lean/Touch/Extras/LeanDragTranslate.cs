...
namespace Lean.Touch
{
	...
	{
		[SerializeField] private float MinimumLimitX = -2f;
		[SerializeField] private float MaximumLimitX = 2f;
		...
		private void Translate(Vector2 screenDelta)
		{
			...
			if (camera != null)
			{
				...
				// Convert back to world space
				newpos = camera.ScreenToWorldPoint(screenPoint);
				transform.position = new Vector3(Mathf.Clamp((newpos.x),MinimumLimitX,MaximumLimitX),transform.position.y,transform.position.z);
